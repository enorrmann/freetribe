...
void encoder_callback(uint8_t index, uint8_t value) {
    ...
    case ENCODER_CUTOFF:
        ...
        if (1 || g_shift_held) { // test
            // module_set_param_all_voices(PARAM_BASE_MORPH_AMOUNT,
            // morph_amount*0.00001f); // also ok
            module_set_param_all_voices(PARAM_PLAYBACK_RATE,
                                        morph_amount); // test
...
void knob_callback(uint8_t index, uint8_t value) {
    ...
    case KNOB_PITCH:
        module_set_param_all_voices(PARAM_TUNE, value);
