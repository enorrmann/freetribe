...
void USB0DeviceIntHandler(void) {
    ...
    /*
     * FIX 3: Removed the inline USBAudio_ProcessOut() call that was here.
     * Audio processing is now centralised in ProcessUSBAudio(), called from
     * app_run only, eliminating the double-processing race.
     */

    /* Clear interrupt in AINTC and OTG wrapper */
...
t_status app_init(void) {
    ...
    /* Disable TI OTG wrapper interrupts – using polling mode */
