...
void ipc_init_buffer() { ipc_buffer_index = 0; }
...
void ipc_send_buffer_chunked(uint32_t total_samples) {
    ...
    uint32_t num_chunks =
        (total_samples + MAX_IPC_TRANSFER_SIZE - 1) / MAX_IPC_TRANSFER_SIZE;
    ft_printf("Total samples: %i, num_chunks: %i", (int)total_samples,
              (int)num_chunks);
    ...
    for (uint32_t i = 0; i < num_chunks; i++) {
        ...
        int status = dev_dsp_ipc_transfer(
            base_address, &ipc_buffer[offset], count, _ipc_callback,
            (void *)0x23AC1D23 // arbitrary user context value for testing
        );

        // ft_printf("status: %d, chunk %u, count: %u, offset: %u, address:
        // 0x%08x", status, i, count, offset, base_address);
