...
static inline int32_t cubic_interp(int32_t y0, int32_t y1, int32_t y2, int32_t y3, uint32_t frac) {
    // frac: Q16.16
    ...
    int64_t t = frac;
    int64_t res =
        (((a0 * t >> FP_SHIFT) + a1) * t >> FP_SHIFT
        + a2) * t >> FP_SHIFT
        + a3;

    return (int32_t)(res >> 1); // normalize
}
static inline int32_t cubic_interp_best(
    ...
    return y1
         + ((int64_t)a * t  >> FP_SHIFT)
         + ((int64_t)b * t2 >> FP_SHIFT)
         + ((int64_t)c * t3 >> FP_SHIFT);
}
static inline uint32_t get_step(uint32_t input_rate) {
    switch (input_rate) {
        case 44100: return STEP_44100;
        case 22050: return STEP_22050;
        case 11025: return STEP_11025;
        default:    return STEP_44100;
    }
}

static inline uint32_t compute_out_len(int in_len, uint32_t input_rate) {
    switch (input_rate) {
        case 44100: return (uint64_t)in_len * 48000 / 44100;
        case 22050: return (uint64_t)in_len * 48000 / 22050;
        case 11025: return (uint64_t)in_len * 48000 / 11025;
        default:    return (uint64_t)in_len * 48000 / 44100;
    }
}
static inline void resample_48000_cubic_best(
    const int32_t *in,
    int in_len,
    int32_t *out,
    uint32_t input_rate
...
static inline void resample_48000_fast(
    const int32_t *in,
    int in_len,
    int32_t *out,
    uint32_t input_rate
